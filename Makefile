...
DOCKER_BE = docker_web_1
DOCKER_DB = docker_db_1
PROJECT_DIR = /home/abrusut/public_html/symfony4-api-rest
UID = $(shell id -u)
...
ssh-be: ## ssh's into the be container
	U_ID=${UID} docker exec -it -u1001 ${DOCKER_BE} bash
...
generate-ssh-keys:	##Generate ssh keys in the container
	U_ID=${UID} docker exec -it ${DOCKER_BE} mkdir -p ${PROJECT_DIR}/config/jwt/
	U_ID=${UID} docker exec -it ${DOCKER_BE} openssl genrsa -out ${PROJECT_DIR}/config/jwt/private.pem -aes256 4096
	U_ID=${UID} docker exec -it ${DOCKER_BE} openssl rsa -pubout -in ${PROJECT_DIR}/config/jwt/private.pem -out ${PROJECT_DIR}/config/jwt/public.pem
